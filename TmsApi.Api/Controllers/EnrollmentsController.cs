    ...
    IEnrollmentService enrollmentService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetEnrollments(int courseId, [FromQuery] PagedRequest request, CancellationToken ct)
        {
            var existing = await courseService.GetByIdAsync(courseId, ct);
            // if not return 404
            if (existing is null)
            {
                return NotFound();
            }
            return Ok(await enrollmentService.GetEnrollmentsAsync(courseId, request, ct));
